...
app.get('/', (req, res) => {
  ...
  for(var i=0; i<tempArr.length; i++){
    ...
    try {
      let queryJSON = JSON.parse(JSON.stringify(req.query))
      if(queryJSON.sendTo){
        
        if(i === parseInt(queryJSON.sendTo)){
          sendMessage(msg, tempArr[i]);
        }
      }else{
        sendMessage(msg, tempArr[i]);  
      }
      if(i === parseInt(queryJSON.endConnection)){
...
const server = createServer(options, app).listen(PORT, () => console.log("Server listening to ", PORT));
...
function onSocketReadable(socket) {
  ...
  //1 - 1 byte - 8bits
  // Reading (2nd byte) and remove first bit from it
